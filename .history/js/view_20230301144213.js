// Если хочешь изменить процентную ставку не лазя в код - замени цифру
let valuePercent = 2.5;
// цифра добавления к сумме числа
let addSums = 3;
...
inputOne.addEventListener('input', () => {
    ...
    let sum = (inputOne.value / 100) * valuePercent;
    let sumres = +inputOne.value + sum + addSums;
...
sum2.addEventListener('input', () => {
    ...
    let sum = (sum2.value / 100) * valuePercent;
    let sumres = +sum2.value + sum + addSums;
    span2.textContent = `${sumres.toFixed(2)} ₴`;
  });
...
phoneSum.addEventListener('input', () => {
    ...
    let sum = (phoneSum.value / 100) * valuePercent;
    let sumres = +phoneSum.value + sum + addSums;
...
phoneSum2.addEventListener('input', () => {
    ...
    let sum = (phoneSum2.value / 100) * valuePercent;
    let sumres = +phoneSum2.value + sum + addSums;
...
requisInputSum.addEventListener('input', () => {
    ...
    let sum = (requisInputSum.value / 100) * valuePercent;
    let sumres = +requisInputSum.value + sum + addSums;
...
requisSum2.addEventListener('input', () => {
    ...
    let sum = (requisSum2.value / 100) * valuePercent;
    let sumres = +requisSum2.value + sum + addSums;
    requisSpan.textContent = `${sumres.toFixed(2)} ₴`;
  });
...
inputSum1.addEventListener('input', () => {
    ...
    let sum = (inputSum1.value / 100) * valuePercent;
    let sumres = +inputSum1.value + sum + addSums;
    inputSum.textContent = `${sumres.toFixed(2)} ₴`;
  });

// получение карт
const cardZsu1 =  document.querySelector('.zsu-card-inp');
const cardZsu2 = document.querySelector('.inp-zsu');
const resultSpanZsu = document.querySelector('.result-zsu');
...
zsuBTN.addEventListener('click', ()=>{
    ...
    cardZsu2.value = cardZsu1.value;
    let sum = (inputSum1.value / 100) * valuePercent;
    let sumres = +inputSum1.value + sum + addSums;
    resultSpanZsu.textContent = `${sumres.toFixed(2)} ₴`;
})

// рассчёт сумы
inputSum2.addEventListener('input', () => {
    if (!inputSum2.value) {
        resultSpanZsu.textContent = "0 ₴";
      return;
    }
    let sum = (inputSum2.value / 100) * valuePercent;
    let sumres = +inputSum2.value + sum + addSums;
    resultSpanZsu.textContent = `${sumres.toFixed(2)} ₴`;
  });